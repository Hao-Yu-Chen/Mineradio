...
async function createWindowOnce() {
  ...
  let startupShellLoadPromise = Promise.resolve();
  const startupShell = path.join(__dirname, 'startup.html');
  if (fs.existsSync(startupShell)) {
    startupShellLoadPromise = win.loadFile(startupShell).catch((error) => {
  ...
  // 等待 startup.html 加载完成，避免后续 loadURL 因并发导航而 ERR_ABORTED 导致闪屏
  await Promise.race([
    startupShellLoadPromise,

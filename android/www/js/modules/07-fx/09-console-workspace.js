...
var FX_CONSOLE_LAYOUT = [
  ...
  },
  {
    ...
    groups: [
      ...
      { key: 'lx-source', title: '获取模式', hint: '默认音源与 LX 源切换、导入和管理', items: [
        fxConsoleItem('lx-mode-seg', '获取模式', '默认 LX 源切换 音源模式'),
        fxConsoleItem('lx-active-source-section', '活跃源', 'LX 活跃音源选择'),
        fxConsoleItem('lx-source-actions', '源管理', '导入在线源 导入本地源'),
        fxConsoleItem('lx-import-row', '导入在线源', 'URL 输入 在线地址'),
        fxConsoleItem('lx-import-local-row', '导入本地源', '文件路径 本地文件'),
        fxConsoleItem('lx-source-list', '已导入源', '源列表 刷新 删除'),
        fxConsoleItem('lx-status-bar', '状态', 'LX 状态 当前模式')
      ] },
      { key: 'experimental', title: '实验功能', hint: '尚未开放或需要谨慎使用的能力', items: [
        fxConsoleItem('t-wallpaperMode', '完整桌面模式', '完整 Mineradio 进入桌面层 Ctrl Shift M 切换操作层 当前启动有效', false)
      ] }
    ]
  }
];

...
// Android: use app internal storage for all paths
const ANDROID_DATA_DIR = (function() {
  try { return require('os').homedir(); } catch(e) {}
  try { return process.env.HOME || '/data/data/com.mineradio.app'; } catch(e) { return '/data/data/com.mineradio.app'; }
})();
...
const COOKIE_FILE = process.env.COOKIE_FILE || path.join(ANDROID_DATA_DIR, 'files', '.cookie');
const QQ_COOKIE_FILE = process.env.QQ_COOKIE_FILE || path.join(ANDROID_DATA_DIR, 'files', '.qq-cookie');
const BEATMAP_CACHE_DIR = process.env.MINERADIO_BEAT_CACHE_DIR || path.join(ANDROID_DATA_DIR, 'files', 'beatmaps');
// Android: no update system (APK updates via app store)
...
// Android: system CA managed by OS, skip manual merge
...
const server = http.createServer(async (req, res) => {
  // Handle CORS preflight for cross-origin requests (e.g. from file:// WebView)
  ...
  // Android: updates handled via APK install, not GitHub
  if (pn === '/api/app/version') {
    sendJSON(res, {
      ...
      version: APP_VERSION,
      mobile: true,
      update: { provider: 'none', configured: false, owner: '', repo: '', preview: false, manifestOverride: false },
    });
    return;
  }

  if (pn === '/api/update/latest') {
    sendJSON(res, { version: APP_VERSION, mobile: true, latest: true, notes: 'Mobile version - updates via APK' });
    return;
  }

  if (pn === '/api/update/download' || pn === '/api/update/download/status' || pn === '/api/update/patch' || pn === '/api/update/patch/status') {
    sendJSON(res, { ok: false, error: 'UPDATE_NOT_AVAILABLE_ON_MOBILE', mobile: true }, 400);
    return;
  }
  ...
  if (pn === '/api/audio') {
    try {
      ...
      // Node.js 12 compatible: use http.get/https.get instead of fetch
      ...
      var up = await new Promise(function(resolve, reject) {
        var upstreamReq = mod.request(reqOpts, function(upRes) {
          // Handle redirects (up to 3)
          if (upRes.statusCode >= 300 && upRes.statusCode < 400 && upRes.headers.location) {
            var redirUrl = new URL(upRes.headers.location, audioUrl).href;
            var rmod = redirUrl.startsWith('https') ? https : http;
            var rparsed = new URL(redirUrl);
            var rreq = rmod.request({
              hostname: rparsed.hostname,
              port: rparsed.port || (rparsed.protocol === 'https:' ? 443 : 80),
              path: rparsed.pathname + rparsed.search,
              headers: hdr,
              timeout: 20000,
            }, function(rr) { resolve({ statusCode: rr.statusCode, headers: rr.headers, stream: rr }); });
            rreq.on('error', function(e) { reject(e); });
            rreq.on('timeout', function() { rreq.destroy(); reject(new Error('Timeout')); });
            rreq.end();
            return;
          }
          resolve({ statusCode: upRes.statusCode, headers: upRes.headers, stream: upRes });
        });
        upstreamReq.on('error', function(e) { reject(e); });
        upstreamReq.on('timeout', function() { upstreamReq.destroy(); reject(new Error('Audio upstream timeout')); });
        upstreamReq.end();
      });
      ...
      var streamTimer = setTimeout(function() {
        if (!streamEnded) { streamEnded = true; try { up.stream.destroy(); } catch(e) {} }
      }, 30000);
      up.stream.on('data', function(chunk) { res.write(chunk); });
      up.stream.on('end', function() {
        streamEnded = true;
        clearTimeout(streamTimer);
        res.end();
      });
      up.stream.on('error', function(streamErr) {
  ...
  // ---------- 静态资源 ----------
  // Android: server.js is in public/nodejs-project/, static files are in public/
  if (pn === '/favicon.ico') {
    // No favicon on Android, return empty
    res.writeHead(204); res.end();
    return;
  }

  let filePath = pn === '/' ? '/index.html' : pn;
  // serve from parent directory (public/)
